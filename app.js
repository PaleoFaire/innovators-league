...
// ─── MAFIA LOOKUP ───
//
// Round 7m helper: the curated FOUNDER_MAFIAS is an object keyed by mafia
// name; the auto-generated FOUNDER_MAFIAS_AUTO is an array of
// {mafia, count, companies:[{company,sector,totalRaised,valuation}],description}.
// Adapt the auto form into the curated object shape, then merge with
// curated-wins so hand-curated detail (founders, color, icon) is preserved.
function getMergedMafias() {
  var curated = (typeof FOUNDER_MAFIAS !== 'undefined') ? FOUNDER_MAFIAS : {};
  var auto = getAutoGlobal('FOUNDER_MAFIAS_AUTO');
  if (!Array.isArray(auto) || !auto.length) return curated;
  var adapted = {};
  auto.forEach(function(m) {
    ...
    adapted[m.mafia] = {
      ...
      companies: (m.companies || []).map(function(c) {
        return { company: c.company, founders: 'Auto-detected from founder/description keywords' };
      }),
      autoGenerated: true
    };
  });
  return (typeof mergeAutoObj === 'function') ? mergeAutoObj(curated, adapted) : curated;
}

function getCompanyMafias(companyName) {
  var source = (typeof getMergedMafias === 'function') ? getMergedMafias() : (typeof FOUNDER_MAFIAS !== 'undefined' ? FOUNDER_MAFIAS : {});
  if (!source) return [];
  const results = [];
  for (const [mafiaName, data] of Object.entries(source)) {
...
function initPodcastSection() {
  ...
  // Round 7m: merge curated FIELD_NOTES with Substack RSS auto-feed
  var notesAll = (typeof mergeAuto === 'function')
    ? mergeAuto(FIELD_NOTES, getAutoGlobal('FIELD_NOTES_AUTO'), function(n) { return n && (n.title || n.id); })
    : FIELD_NOTES;

  var podcasts = notesAll.filter(function(n) { return n.type === 'podcast' || n.type === 'interview'; })
...
function initGlobalSearch() {
  ...
  searchInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && searchInput.value.trim()) {
      ...
      // Scroll to first result or company section
...
function initFieldNotes() {
  ...
  // Round 7m: merge in Substack RSS auto-feed so the grid stays fresh
  // between hand-curated notes.
  var mergedNotes = (typeof mergeAuto === 'function')
    ? mergeAuto(FIELD_NOTES, getAutoGlobal('FIELD_NOTES_AUTO'), function(n) { return n && (n.title || n.id); })
    : FIELD_NOTES;

  // Show the latest 6 notes
  const notes = mergedNotes.slice(0, 6);
...
function initMafiaExplorer() {
  ...
  // Round 7m: include auto-detected mafias (Y Combinator, MIT CSAIL, etc.)
  var source = (typeof getMergedMafias === 'function') ? getMergedMafias() : FOUNDER_MAFIAS;
...
function initGovContracts() {
  ...
  if (typeof CONTRACTOR_READINESS !== 'undefined' && readinessPanel) {
    // Round 7m: merge curated rows with auto-computed readiness (0-100 scale,
    // derived from govTraction + contract counts + ITAR/clearance keywords).
    const mergedReadiness = (typeof mergeAuto === 'function')
      ? mergeAuto(CONTRACTOR_READINESS, getAutoGlobal('CONTRACTOR_READINESS_AUTO'), function(r) { return r && r.company; })
      : CONTRACTOR_READINESS;
    const sorted = [...mergedReadiness].sort((a, b) => b.readinessScore - a.readinessScore);
  ...
  // Budget Signals Panel (Round 7m: merge curated + auto sector-activity signals)
  if (typeof BUDGET_SIGNALS !== 'undefined' && budgetPanel) {
    const mergedBudget = (typeof mergeAuto === 'function')
      ? mergeAuto(BUDGET_SIGNALS, getAutoGlobal('BUDGET_SIGNALS_AUTO'), function(b) { return b && b.category; })
      : BUDGET_SIGNALS;
    budgetPanel.innerHTML = mergedBudget.map(b => {
      const change = b.change || '0%';
      const changeCls = change.startsWith('-') ? 'negative' : 'positive';
      const beneficiaries = Array.isArray(b.beneficiaries) ? b.beneficiaries : [];
      const benefitHTML = beneficiaries.length
        ? `<div class="budget-beneficiaries">
             <span class="budget-beneficiaries-label">Beneficiaries:</span>
             ${beneficiaries.map(name => `<span class="budget-beneficiary" onclick="openCompanyModal('${name.replace(/'/g, "\\'")}')">${name}</span>`).join('')}
           </div>`
        : '';
      return `
        <div class="budget-signal-card">
          <div class="budget-signal-header">
            ...
            <span class="budget-change ${changeCls}">${change}</span>
          </div>
          <div class="budget-line-item">${b.budgetLineItem || ''}</div>
          <div class="budget-allocation">${b.allocation || 'TBD'} · ${b.fy || ''}</div>
...
function initPredictiveScoring() {
  ...
  function renderMAComps() {
    ...
    // Round 7m: merge curated comps with auto-extracted ones from news/press/SEC.
    // Auto comps use `ev` instead of `dealValue/evRevenue`; normalize the shape
    // so the same row template works for both.
    var autoRaw = getAutoGlobal('MA_COMPS_AUTO') || [];
    var autoAdapted = autoRaw.map(function(c) {
      return {
        target: c.target,
        acquirer: c.acquirer,
        sector: c.sector,
        year: c.year,
        dealValue: c.ev || c.dealValue || '',
        evRevenue: c.evRevenue || 'N/A',
        evFunding: c.evFunding || 'N/A',
        type: c.type || 'Announced',
        notes: c.notes || '',
        source: c.source || '',
        url: c.url || '',
        autoGenerated: true
    ...
    var merged = (typeof mergeAuto === 'function')
      ? mergeAuto(MA_COMPS, autoAdapted, function(x) { return x && (x.target || '').toLowerCase() + '|' + (x.acquirer || '').toLowerCase(); })
      : MA_COMPS;
...
function generateNetworkData() {
  ...
  // 5b. Round 7m: add competitor/thesis edges from NETWORK_GRAPH_AUTO
  //     (ids there are bare company names, e.g. "Anduril Industries"; our
  //     company-node ids are prefixed "company-<name>"). Only add edges
  //     whose endpoints we already have as company nodes.
  const autoGraph = getAutoGlobal('NETWORK_GRAPH_AUTO');
  if (autoGraph && Array.isArray(autoGraph.edges)) {
    autoGraph.edges.forEach(e => {
      if (!e || !e.src || !e.dst) return;
      if (e.type !== 'competitor' && e.type !== 'thesis') return;
      const srcId = 'company-' + e.src;
      const tgtId = 'company-' + e.dst;
      if (!nodeIds.has(srcId) || !nodeIds.has(tgtId)) return;
      // Dedupe against existing edges in either direction
      const dup = edges.find(x => (x.source === srcId && x.target === tgtId) || (x.source === tgtId && x.target === srcId));
      if (dup) return;
      edges.push({
        source: srcId,
        target: tgtId,
        type: e.type,
        detail: e.type === 'thesis' ? ('Shared thesis: ' + (e.cluster || '')) : 'Competitors'
      });
    });
  }
...
function initOpti() {
  ...
  function buildMafiaResponse(q) {
    ...
    // Round 7m: use merged curated + auto mafia clusters for richer answers
    const mergedSource = (typeof getMergedMafias === 'function') ? getMergedMafias() : FOUNDER_MAFIAS;
